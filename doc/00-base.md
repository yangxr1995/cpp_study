...
```c
int* p = &a;
const int*& q = p;  // 错误！无法将int**隐式转为const int**
                    // 错误原因：C++不允许通过隐式转换将非const指针的引用绑定到const指针上，这会破坏类型安全。
// 正确做法：
int*& q = p;        // 正确：类型完全匹配
// 或显式强制转换
const int* const& qq = p;  // 正确但语义不同
```
### 正确的一级指针赋值
```c
int* a;
...
```
3. 警惕隐式转换
当出现const与非const混合使用时，优先使用static_cast显式转换，避免编译器隐式生成临时对象。
